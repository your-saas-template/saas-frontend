...
export const FiltersBar: React.FC<FiltersBarProps> = ({
  ...
  const gridClass = searchOnlyMode
    ? "w-full"
    : hasSort && hasExtraFilters
    ? "grid gap-3 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]"
    : hasSort
    ? "grid gap-3 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_auto]"
  ...
  return (
    ...
    >
      <div className={searchOnlyMode ? "w-full" : "flex-1 min-w-0"}>
        ...
        {(hasClearButton || hasSort) && (
          <div className="mt-3 flex items-center gap-2 lg:hidden">
            {hasSort && <div className="flex shrink-0">{sortControl}</div>}
            {hasClearButton && (
              <Button
                type="button"
      ...
      {hasClearButton && (
        <div className="hidden items-center justify-end lg:flex lg:w-auto">
          <Button
            type="button"
            variant={ButtonVariantEnum.primary}
            onClick={handleClearAll}
